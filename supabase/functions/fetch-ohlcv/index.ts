...
serve(async (req) => {
  ...
  try {
    // Parse mode first and start background job
    ...
    // Run the heavy fetch in background to avoid client timeouts / hangs
    const er = (globalThis as any).EdgeRuntime;
    const runInBackground = (promise: Promise<any>) => {
      if (er?.waitUntil) {
        er.waitUntil(promise);
      } else {
        promise.catch((e) => console.error('[FetchOHLCV] Background task error (no waitUntil):', e));
      }
    };
    runInBackground((async () => {
      try {
        ...
        if (!TWELVE_DATA_KEY) {
          console.error('[FetchOHLCV] Twelve Data API key not configured');
          return;
        }
        ...
        console.log(`[FetchOHLCV] Starting OHLCV fetch (${modeLabel} режим) [background]`);
        ...
        for (const symbol of pairs) {
          for (const [timeframe, tfConfig] of Object.entries(config)) {
            try {
              ...
              const fetchCount = needsLoad ? (tfConfig as any).count : 2;
              ...
              const url = `https://api.twelvedata.com/time_series?symbol=${encodeURIComponent(symbol)}&interval=${(tfConfig as any).interval}&outputsize=${fetchCount}&apikey=${encodeURIComponent(TWELVE_DATA_KEY)}`;
              ...
              // Smart delay: 
              // LOAD (перше завантаження) - 15 сек (безпечно для API)
        ...
        const failCount = results.filter((r: any) => r.status !== 'success').length;
        const loadCount = results.filter((r: any) => r.mode === 'LOAD').length;
        const updateCount = results.filter((r: any) => r.mode === 'UPDATE').length;
        ...
        console.error('[FetchOHLCV] Fatal error in background:', error);
      }
    })());
    ...
    return new Response(
      JSON.stringify({ 
        success: true,
        message: `🔄 ${modeLabel}: фонове завантаження свічок запущено. Оновлення індикаторів виконається автоматично.`,
      }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
    );

  } catch (error) {
    console.error('[FetchOHLCV] Fatal error (request):', error);
    return new Response(
      JSON.stringify({ 
